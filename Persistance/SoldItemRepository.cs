...
namespace Persistance
{
    public class SoldItemRepository : ISoldItemRepository
    {
        ...
        {
            var item = await Context.SoldItem.FindAsync(Id);
            return item;
        }

        public async Task<List<SoldItem>> GetAll()
        {
            var items = await Context.SoldItem.Select(x => x).ToListAsync();
            return items;
        }

        public async Task<List<SoldItem>> GetSoldLast30Days() {
            var firstDay = DateTime.Today.AddDays(-30);
            var items = await Context.SoldItem.Where(x => x.TimeStamp >= firstDay)
                            .ToListAsync();
            return items;
        }
